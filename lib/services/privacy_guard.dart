...
class PrivacyGuard {
  ...
  static Future<void> initialize() async {
    if (!_isConsentChecked) {
      ...
      _hasValidConsent =
          _cachedConsent?.isFullyConsented == true &&
          !(_cachedConsent?.needsUpdate ?? true);
      _isConsentChecked = true;

      _logger.i('🔒 PrivacyGuard initialized: consent=${_hasValidConsent}');
    }
  }
  ...
  static bool hasValidConsent() {
    if (!_isConsentChecked) {
      _logger.w('⚠️ PrivacyGuard not initialized, blocking access');
      return false;
    }
  ...
  static void updateConsent(PolicyConsent consent) {
    ...
    _logger.i('🔓 PrivacyGuard consent updated: ${_hasValidConsent}');
  }
  ...
  static void revokeConsent() {
    ...
    _cachedConsent = null;
    _logger.i('🔒 PrivacyGuard consent revoked');
  }

  /// 检查特定功能的访问权限
  static bool canAccess(PrivacyFeature feature) {
    if (!hasValidConsent()) {
      _logger.w('🚫 Access denied for ${feature.name}: no consent');
      return false;
    }
  ...
  static Future<T?> guardedExecution<T>({
    ...
    if (!canAccess(feature)) {
      _logger.w(
        '🚫 Blocked execution of ${feature.name}: ${errorMessage ?? 'No consent'}',
      );
      return null;
    }
    ...
    try {
      ...
      _logger.e('❌ Error in guarded execution for ${feature.name}: $e');
      return null;
    }
  }
  ...
  static T? guardedSync<T>({
    ...
    if (!canAccess(feature)) {
      _logger.w(
        '🚫 Blocked sync execution of ${feature.name}: ${errorMessage ?? 'No consent'}',
      );
      return null;
    }
    ...
    try {
      ...
      _logger.e('❌ Error in guarded sync execution for ${feature.name}: $e');
      return null;
    }
  }
  ...
  static void logAccess(
    ...
    _logger.i(
      '📝 Privacy Access Log: ${feature.name} - $status${details != null ? ' ($details)' : ''}',
    );
  }
...
class PrivacyFutureBuilder<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (!PrivacyGuard.canAccess(feature)) {
      return noConsentWidget ?? Container(child: const Text('隐私权限不足'));
    }
