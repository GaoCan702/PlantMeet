...
class LLMPerformanceOptimizer {
  ...
  static Future<File> optimizeImageForLLM(File originalFile) async {
    try {
      ...
      // 输出优化信息
      _logger.i('🖼️ 图片优化完成:');
      _logger.i('  原始: $originalDimensions (${_formatFileSize(originalSize)})');
      _logger.i(
        '  优化: ${image.width}x${image.height} (${_formatFileSize(optimizedBytes.length)})',
      );
      _logger.i(
        '  压缩率: ${((1 - optimizedBytes.length / originalSize) * 100).toStringAsFixed(1)}%',
      );

      return optimizedFile;
    } catch (e) {
      _logger.w('⚠️ 图片优化失败，使用原始文件: $e');
      return originalFile;
    }
  }
...
''';
  ...
  static img.Image _createTestImage() {
    ...
    img.drawCircle(
      ...
      y: 200,
      radius: 50,
      color: img.ColorRgb8(200, 100, 100),
    );
    img.drawRect(
      image,
      x1: 200,
      ...
      for (final file in files) {
        if (file is File &&
            (file.path.contains('optimized_') ||
                file.path.contains('test_image_'))) {
          try {
            await file.delete();
          } catch (e) {
            // 忽略删除失败的文件
  ...
  static List<String> getPerformanceTips() {
    ...
    if (isLowEnd) {
      tips.addAll(['设备性能较低，建议使用快速识别模式', '避免同时处理多张图片', '定期清理应用缓存以释放内存']);
    } else {
      tips.addAll(['设备性能良好，可使用详细识别模式', '支持批量处理和高质量输出', '建议开启本地缓存以提高响应速度']);
    }

    tips.addAll(['拍摄植物时保持清晰聚焦', '避免强烈逆光和阴影', '包含植物的关键特征（叶片、花朵等）']);
