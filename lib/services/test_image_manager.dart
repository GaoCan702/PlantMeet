...
class TestImageManager {
  ...
  static Future<void> initializePresetImages() async {
    ...
    for (final imageInfo in _presetImages) {
      ...
      if (!await file.exists()) {
        ...
        _logger.i('✅ 生成测试图片: ${imageInfo.name}');
      }
    }
  ...
  static img.Image _createRoseFlower() {
    ...
    img.drawCircle(
      ...
      y: 256,
      radius: 15,
      color: img.ColorRgb8(220, 200, 80),
    );
  ...
  static img.Image _createCactusPlant() {
    ...
    for (int i = 0; i < 50; i++) {
      ...
      img.drawCircle(
        ...
        y: y,
        radius: 3,
        color: img.ColorRgb8(140, 120, 80),
      );
    ...
    img.drawCircle(
      ...
      y: 180,
      radius: 8,
      color: img.ColorRgb8(220, 100, 140),
    );
  ...
  static img.Image _createComplexScene() {
    ...
    img.drawCircle(
      ...
      y: 200,
      radius: 25,
      color: img.ColorRgb8(200, 150, 100),
    );
  ...
  static Future<int> getImageFileSize(String imageId) async {
    try {
      ...
      _logger.e('获取图片大小失败: $e');
    }
    return 0;
  }

  // 辅助绘图方法
  static void _drawHeartShapeLeaf(
    img.Image image,
    int centerX,
    int centerY,
    int size,
    ...
    for (int i = -size; i < size; i++) {
      for (int j = -size; j < size; j++) {
        ...
        // 心形方程的简化版本
        if ((x * x + y * y - 1).abs() < 0.3 ||
            (x * x + (y - 0.5) * (y - 0.5) - 0.5).abs() < 0.2) {
