...
class LocalLLMService {
  ...
  Future<bool> initialize() async {
    try {
      ...
      _logger.e('LocalLLMService initialization failed: $e');
      return false;
    }
  }
  ...
  Future<String> _callVisionModel({
    ...
    final request = {
      'model': modelName,
      'messages': [
        {'role': 'system', 'content': LLMPromptTemplates.systemPrompt},
        {
          'role': 'user',
          'content': [
    ...
    if (response.statusCode != 200) {
      throw Exception(
        'Model inference failed: ${response.statusCode} - ${response.body}',
      );
    }
  ...
  /// 解析模型响应并转换为应用数据结构
  Future<RecognitionResponse> _parseModelResponse(
    String response,
    bool quickMode,
  ) async {
    try {
  ...
  RecognitionResult _parseDetailedResult(Map<String, dynamic> data) {
    return RecognitionResult(
      ...
      care: data['care_tips'] != null
          ? CareInfo.fromJson(data['care_tips'])
          : null,
  ...
  Future<Map<String, dynamic>> getModelInfo() async {
    try {
      ...
      return {'error': e.toString(), 'model': modelName, 'status': 'unknown'};
    }
  }
